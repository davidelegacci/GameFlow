...
class LinFTRL():
    """docstring for Akin"""
    def __init__(self, simkernels, reglabels, radius, sim_manual_regularizers = 0):
        super(LinFTRL, self).__init__()

        self.radius = radius

        self.sim_manual_regularizers = sim_manual_regularizers
        ...
        if sim_manual_regularizers: self.simregularizers += self.sim_manual_regularizers
        #-------------------------------------------------------------------------------------------
        self.simregularizers1_tmp = [sp.diff(h, x) for h in self.simregularizers]
        self.simregularizers1 = [sp.simplify(h1) for h1 in self.simregularizers1_tmp]

        self.simregularizers2 = [sp.diff(h1, x) for h1 in self.simregularizers1]
        self.simregularizers3 = [sp.diff(h2, x) for h2 in self.simregularizers2]
        ...
        self.regularizers3 = [sp.lambdify(x, h3) for h3 in self.simregularizers3]

        # Differential term appearing in co-exact PDE
        self.simPDE = [  sp.simplify(  self.simregularizers3[i] / (self.simregularizers2[i]**2)  ) for i, h in enumerate(self.simregularizers)  ]
        self.PDE = [sp.lambdify(x, pde) for pde in self.simPDE]
    ...
    def plot_method(self, functions, simfunctions, **kwargs):
        ...
        for i, f in enumerate(functions):
            ...
            if f(0.1) ==  f(0.2)  and f(0.2) == f(0.3):
                plt.plot(xDomain, np.ones(len(xDomain)) * f(0.1), label = f"{self.reglabels[i]}: ${sp.latex(  self.round_ex(simfunctions[i])  )}$ " )

            else:
                plt.plot(xDomain, f(xDomain), label = f"{self.reglabels[i]}: ${sp.latex(  self.round_ex(simfunctions[i])  )}$ " )
